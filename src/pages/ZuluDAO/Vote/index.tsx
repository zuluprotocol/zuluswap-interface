...
import luxuryGreenBackground from 'assets/images/zuludao/luxury-green-background-small.jpg'
...
import { useClaimRewardActions, useVotingActions, useVotingInfo } from 'hooks/zuludao'
...
import SwitchToEthereumModal, { useSwitchToEthereum } from '../StakeZPX/SwitchToEthereumModal'
...
export default function Vote() {
  ...
  const { zpx, usd, zpxPriceETH } = useTotalVotingReward()
  ...
  const toggleClaimConfirmModal = useToggleModal(ApplicationModal.ZULU_DAO_CLAIM)
  ...
  const handleClaim = useCallback(() => {
    switchToEthereum().then(() => {
      mixpanelHandler(MIXPANEL_TYPE.ZULU_DAO_CLAIM_CLICK)
  ...
  const handleConfirmClaim = useCallback(async () => {
    ...
    setPendingText(t`Claming ${formatUnitsToFixed(remainingCumulativeAmount)} ZPX`)
    setShowConfirm(true)
          ...
          <Card>
            <AutoColumn>
              <Text color={theme.subText} marginBottom="20px">
                <Trans>Total Staked ZPX</Trans>
              </Text>
              <Text fontSize={20} marginBottom="8px" fontWeight={500}>
                {daoInfo ? formattedNumLong(Math.round(daoInfo.total_staked)) + ' ZPX' : '--'}
              </Text>
              <Text fontSize={12} color={theme.subText}>
                {daoInfo && zpxPriceETH
                  ? '~' + formattedNumLong(zpxPriceETH * Math.round(daoInfo.total_staked)) + ' USD'
                  : ''}
              </Text>
            </AutoColumn>
          </Card>
...
[Your Staked ZPX] / [Total Staked ZPX] * 100%`}
                />
              </Text>
                ...
                <RowFit>
                  ...
                  {!totalStakedAmount ? (
                    <InfoHelper
                      placement="top"
                      fontSize={12}
                      text={t`You have to stake ZPX to be able to vote and earn voting reward`}
                    />
                  ) : null}
                </RowFit>
                {isDelegated && (
              ...
              <RowBetween>
                <Text fontSize={12} color={theme.subText}>
                  {totalStakedAmount ? (+totalStakedAmount.toFixed(2)).toLocaleString() + ' ZPX Staked' : '--'}
                </Text>
                <StyledInternalLink to="/zuludao/stake-zpx" style={{ fontSize: '12px', whiteSpace: 'nowrap' }}>
            ...
            <AutoColumn justify="space-between">
              ...
              {account ? (
                <RowBetween>
                  <AutoColumn>
                    <Text fontSize={20} marginBottom="8px" fontWeight={500}>
                      {formatUnitsToFixed(remainingCumulativeAmount)} ZPX
                    </Text>
                    <Text fontSize={12} color={theme.subText}>
                      {(+formatUnitsToFixed(remainingCumulativeAmount) * +(zpxPriceETH || '0')).toFixed(2)} USD
                    </Text>
                  </AutoColumn>
                  <ButtonPrimary
                    width="75px"
        ...
        <Text color={theme.subText} fontStyle="italic" fontSize={12} hidden={isMobile}>
          <Trans>Note: Voting on ZuluDAO is only available on Ethereum chain</Trans>
        </Text>
