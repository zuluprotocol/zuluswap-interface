...
export default function ClaimConfirmModal({ amount, onConfirmClaim }: { amount: string; onConfirmClaim: () => void }) {
  ...
  const modalOpen = useModalOpen(ApplicationModal.ZULU_DAO_CLAIM)
  const toggleModal = useToggleModal(ApplicationModal.ZULU_DAO_CLAIM)
  return (
    <Modal isOpen={modalOpen} onDismiss={toggleModal}>
      <Wrapper>
        <RowBetween>
          <AutoRow gap="2px">
            <Text fontSize={20}>
              <Trans>Claim your ZPX rewards</Trans>
            </Text>
          </AutoRow>
        ...
        <RowFit gap="10px">
          <ZPXLogo size={28} /> <Text fontSize={32}>{amount} ZPX</Text>
        </RowFit>
