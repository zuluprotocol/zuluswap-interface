...
import { useZuluDAOInfo, useZuluDaoStakeActions, useStakingInfo, useVotingInfo } from 'hooks/zuludao'
import { ApprovalState, useApproveCallback } from 'hooks/useApproveCallback'
import useMixpanel, { MIXPANEL_TYPE } from 'hooks/useMixpanel'
import useTheme from 'hooks/useTheme'
import { ApplicationModal } from 'state/application/actions'
import { useZPXPrice, useToggleModal, useWalletModalToggle } from 'state/application/hooks'
...
const StakeFormWrapper = styled.div`
  ...
  margin-bottom: 16px;
`
const YourStakedZPX = styled(FormWrapper)`
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 16px;
...
export const ZPXLogoWrapper = styled.div`
  border-radius: 20px;
...
const GetZPXButton = styled(Link)`
  display: flex;
  align-items: center;
...
export default function StakeZPXComponent() {
  const theme = useTheme()
  const { account, chainId } = useActiveWeb3React()
  const zuluDAOInfo = useZuluDAOInfo()
  const { stakedBalance, ZPXBalance, delegatedAddress } = useStakingInfo()
  const { calculateVotingPower } = useVotingInfo()
  const isDelegated = !!delegatedAddress && delegatedAddress !== account
  const { stake, unstake, delegate, undelegate } = useZuluDaoStakeActions()
  ...
  useEffect(() => {
    ...
    if (!inputValue || isNaN(parseFloat(inputValue)) || parseFloat(inputValue) <= 0) {
      ...
      (parseFloat(inputValue) > parseFloat(formatUnits(ZPXBalance)) && activeTab === STAKE_TAB.Stake) ||
  ...
  const toggleYourTransactions = useToggleModal(ApplicationModal.YOUR_TRANSACTIONS_STAKE_ZPX)
  const { switchToEthereum } = useSwitchToEthereum()
  const { mixpanelHandler } = useMixpanel()
  const [approvalZPX, approveCallback] = useApproveCallback(
    TokenAmount.fromRawAmount(
      new Token(chainId === ChainId.GÖRLI ? ChainId.GÖRLI : ChainId.MAINNET, zuluDAOInfo?.ZPXAddress || '', 18, 'ZPX'),
      MaxUint256,
    ),
    zuluDAOInfo?.staking,
  )
  ...
  const handleStake = () => {
    switchToEthereum()
      .then(() => {
        setPendingText(t`Staking ${inputValue} ZPX to ZuluDAO`)
        setShowConfirm(true)
        setAttemptingTxn(true)
        mixpanelHandler(MIXPANEL_TYPE.ZULU_DAO_STAKE_CLICK, { amount: inputValue })
      ...
      .catch(() => setFeatureText(t`Staking ZPX`))
  }

  const handleUnstake = () => {
    switchToEthereum()
      .then(() => {
        setPendingText(t`Unstaking ${inputValue} ZPX from ZuluDAO`)
        setShowConfirm(true)
        setAttemptingTxn(true)
        mixpanelHandler(MIXPANEL_TYPE.ZULU_DAO_UNSTAKE_CLICK, { amount: inputValue })
      ...
      .catch(() => setFeatureText(t`Unstaking ZPX`))
  }
  ...
  const onDelegateConfirmed = useCallback(() => {
    ...
    if (isUndelegate.current) {
      ...
      setShowConfirm(true)
      setAttemptingTxn(true)
      mixpanelHandler(MIXPANEL_TYPE.ZULU_DAO_DELEGATE_CLICK, { delegateAddress: delegateAddress })
  ...
  const zpxPrice = useZPXPrice()
  const zpxValueInUsd = useMemo(() => {
    if (!zpxPrice || !inputValue) return 0
    return (parseFloat(zpxPrice) * parseFloat(inputValue)).toFixed(2)
  }, [zpxPrice, inputValue])

  const handleMaxClick = useCallback(
    (half?: boolean) => {
      const balance = activeTab === STAKE_TAB.Stake ? ZPXBalance : stakedBalance
      setInputValue(formatUnits(balance.div(!!half ? 2 : 1)))
    },
    [activeTab, ZPXBalance, stakedBalance],
  )
    ...
    <Wrapper>
      ...
      <YourStakedZPX>
        ...
        <Text
          ...
          style={{ gap: '8px' }}
        >
          <ZPXLogo size={20} /> {formatUnits(stakedBalance)} ZPX
        </Text>
      </YourStakedZPX>
      ...
      <StakeFormWrapper>
        <StakeForm>
          <RowBetween color={theme.subText}>
            <GetZPXButton to="/swap/ethereum/eth-to-zpx">
              <Repeat size={16} />
              <Text fontSize={14}>
                <Trans>Get ZPX</Trans>
              </Text>
          ...
          {(activeTab === STAKE_TAB.Stake || activeTab === STAKE_TAB.Unstake) && (
            <>
                ...
                <RowBetween width={'100%'}>
                  ...
                  {activeTab === STAKE_TAB.Stake && (
                    <AutoRow gap="3px" justify="flex-end" color={theme.subText}>
                      <Wallet /> <Text fontSize={12}>{ZPXBalance ? formatUnits(ZPXBalance) : 0}</Text>
                    </AutoRow>
                  )}
                </RowBetween>
                <RowBetween>
                  <Input value={inputValue} onUserInput={setInputValue} />
                  <span style={{ color: theme.border, fontSize: '14px', marginRight: '6px' }}>~${zpxValueInUsd}</span>
                  <ZPXLogoWrapper>
                    <ZPXLogo />
                    ZPX
                  </ZPXLogoWrapper>
              ...
              {account ? (
                <Row gap="12px">
                  {(approvalZPX === ApprovalState.NOT_APPROVED || approvalZPX === ApprovalState.PENDING) &&
                    [ChainId.MAINNET, ChainId.GÖRLI].includes(chainId) &&
                    !errorMessage && (
                      <ButtonPrimary onClick={approveCallback} disabled={approvalZPX === ApprovalState.PENDING}>
                        {approvalZPX === ApprovalState.PENDING ? 'Approving...' : 'Approve'}
                      </ButtonPrimary>
                    )}
                  <ButtonPrimary
                    disabled={
                      [ChainId.MAINNET, ChainId.GÖRLI].includes(chainId) &&
                      (approvalZPX !== ApprovalState.APPROVED || !!errorMessage)
                    }
                    margin="8px 0px"
                    onClick={() => {
                ...
                <ButtonLight onClick={toggleWalletModal}>
                  <InfoHelper
                    ...
                    fontSize={12}
                    color={theme.primary}
                    text={t`Staking ZPX is only available on Ethereum chain`}
              ...
              <ExpandableBox
                ...
                headerContent={
                  <AutoRow>
                    ...
                    <Text fontSize={12} lineHeight="16px" color={theme.subText}>
                      <Trans>Important Notice: Zulu Network does not hold your funds or manage this process.</Trans>
                    </Text>
                  </AutoRow>
                }
                expandContent={
                  <Text margin={'0 20px 0 30px'} fontSize={12} lineHeight="16px">
                    <Trans>
                      ...
                      distributing your ZPX rewards to you, though only you can withdraw/unstake your own ZPX
                    </Trans>
                  </Text>
                }
              />
          ...
          <AutoColumn gap="10px" style={{ fontSize: '12px' }}>
            ...
            <RowBetween>
              <Text>
                ...
                <InfoHelper
                  text={t`Your voting power is calculated by [Your Staked ZPX] / [Total Staked ZPX] * 100%`}
                />
              </Text>
              <Text>
                {currentVotingPower}%
