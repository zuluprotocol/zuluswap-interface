...
import { useStakingInfo, useVotingInfo } from 'hooks/zuludao'
import { ActionType, StakerAction } from 'hooks/zuludao/types'
...
export default function YourTransactionsModal() {
  ...
  const modalOpen = useModalOpen(ApplicationModal.YOUR_TRANSACTIONS_STAKE_ZPX)
  const toggleModal = useToggleModal(ApplicationModal.YOUR_TRANSACTIONS_STAKE_ZPX)
            ...
            {formattedActions.length > 0 ? (
              ...
              <Flex alignItems="center" justifyContent="center" flex={1} flexDirection="column" style={{ gap: '10px' }}>
                ...
                <Text>
                  <Trans>
                    Go to{' '}
                    <Link to="/zuludao/stake-zpx" onClick={() => toggleModal()}>
                      Stake
                    </Link>
                  </Trans>
                </Text>
