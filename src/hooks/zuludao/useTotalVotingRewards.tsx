...
import { getEthPrice, getZPXPriceByETH, useZuluSwapConfig } from 'state/application/hooks'
...
const LP_ADDRESSES = [
  '0x61639d6ec06c13a96b5eb9560b359d7c648c7759', // eth-zpx
  ...
  '0x38ff2ea1a930478f002af766e63774fc02f04fdf', //zpx-und
...
const MATIC_LP_ADDRESSES = [
  ...
  '0x37e6449b0e99befd2a708ea048d970f4ff4dc65d', //matic-zpx
  ...
  '0xd8b9e9444fcbf26bea4badd6142dd6a962bca86a', //zpx-eth
...
export default function useTotalVotingReward(): {
  ...
  const [zpxPriceETH, setZpxPriceETH] = useState(0)
  const { classicClient: classicClientMainnet, blockClient, provider } = useZuluSwapConfig(ChainId.MAINNET)
  const { classicClient: classicClientMatic, provider: providerMatic } = useZuluSwapConfig(ChainId.MATIC)

  const [localStoredTotalVotingReward, setLocalStoredTotalVotingReward] = useLocalStorage(
    'zuludao-totalVotingRewards',
    0,
  )
    ...
    const run = async () => {
      try {
        ...
        const rewards = await Promise.all([
          ...
          (async () => {
            const ZPXContract = new Contract(ZPX_ADDRESS, ERC20_ABI, provider)
            const zpxBalance = await ZPXContract.balanceOf(REWARD_POOL_ADDRESS)
            return parseFloat(new Fraction(zpxBalance.toString(), 10 ** 18).toSignificant(18))
          })(),
        ])
  ...
  useEffect(() => {
    async function checkForZPXPrice() {
      const zpxPriceByETH = await getZPXPriceByETH(ChainId.MAINNET, classicClientMainnet)
      const ethPrice = await getEthPrice(ChainId.MAINNET, classicClientMainnet, blockClient)
      const zpxPrice = zpxPriceByETH * ethPrice[0] || 0
      setZpxPriceETH(zpxPrice)
    }
    checkForZPXPrice()
  ...
  return {
    ...
    zpx: useMemo(
      () =>
        zpxPriceETH && zpxPriceETH !== 0
          ? Math.floor((totalVotingReward || localStoredTotalVotingReward || 0) / zpxPriceETH)
          : 0,
      [totalVotingReward, zpxPriceETH, localStoredTotalVotingReward],
    ),
    zpxPriceETH,
  }
}
